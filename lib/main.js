...
let {Cc, Ci, Cu, Cr, Cm, components: {ID: CI} } = require("chrome");
...
AboutConfMania.prototype = {
  ...
  classID: CI("{da341d45-077c-47e2-a9a6-fea161c222b3}"),
...
function openConfMania(aWindow) {
  if ((Services.prefs.getPrefType('browser.preferences.inContent') == Ci.nsIPrefBranch.PREF_BOOL)
   && (Services.prefs.getBoolPref('browser.preferences.inContent') == false)) {
    ...
    aWindow.switchToTabHavingURI("about:confmania", true, {ignoreFragment: true});
  }
}
  ...
  while (enu.hasMoreElements()) {
    ...
    while (containedDocShells.hasMoreElements()) {
      ...
      if ((childDoc.location.href.indexOf("chrome://confmania/") == 0)
          || (childDoc.location.href.indexOf("about:confmania") == 0)) {
...
exports.startup = function startup(options) {
  ...
  if (options.loadReason === "install" || options.loadReason === "enable") {
    let userPrefs = {};
    for (let fileName of ["paneBrowser.xul", "paneSecurity.xul", "paneHTTP.xul", "paneUI.xul", "paneAddons.xul", "paneDebug.xul"]) {
      let req = Cc["@mozilla.org/xmlextras/xmlhttprequest;1"]
                  .createInstance(Ci.nsIXMLHttpRequest);
      req.open("GET", "chrome://confmania/content/" + fileName, false);
      req.send(null);
      let doc = req.responseXML;
      for (let v of doc.querySelectorAll("preference[name]")) {
...
exports.shutdown = function shutdown(reason) {
  ...
  if (reason === "uninstall" || reason === "disable") {
    ...
    switch (buttonPressed) {
      ...
      default: // Cancel
        ...
        am.AddonManager.getAddonByID(require("sdk/self").id, function (addon) {
          if (addon) {
...
exports.browserWindowStartup = function browserWindowStartup(params, aWindow) {
...
exports.browserWindowShutdown = function browserWindowShutdown(params, aWindow) {
  let document = aWindow.document;

  for (let id of ["ConfMania:Open", "confmania", "appmenu_confmania"]) {
